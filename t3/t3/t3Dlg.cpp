...
Ct3Dlg::Ct3Dlg(CWnd* pParent /*=NULL*/)
	: CDialogEx(Ct3Dlg::IDD, pParent)
{
...
BEGIN_MESSAGE_MAP(Ct3Dlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_GO, &Ct3Dlg::OnBnClickedGo)
END_MESSAGE_MAP()


// Ct3Dlg message handlers
...
void Ct3Dlg::RemoveDuplicates(CString &input)
{
	CString output;
...
void Ct3Dlg::Expand(CString &input)
{
	CString output;
	for (int i = 0; i < input.GetLength(); i++)
	{
...
void Ct3Dlg::CalculateDual(const CString &input, CString &dual)
{
	dual.Empty();
	for (int i = 0; i < input.GetLength(); i++)
	{
...
void Ct3Dlg::th_LoadDataFiles(void *p)
{
	Ct3Dlg *pThis = (Ct3Dlg *) p;
	for (int i = 13; i < 14; i++)
	{
		char buf[80];
...
CString Ct3Dlg::Separated(const CString &input)
{
	CString s;
	for (int i = 0; i < input.GetLength(); i++)
	{
...
void Ct3Dlg::Check(int r, int &major, int &minor)
{
	if (IsMajor(r))
		major++;
	else if (IsMinor(r))
...
void Ct3Dlg::Calc(const CString &s, bool bRev, int memberOffset, int memberSize, CeOperation op, int &major, int &minor)
{
...
void Ct3Dlg::CalculateScore(const CString &s, int &major, int &minor)
{
	major = minor = 0;
...
void Ct3Dlg::AddSentence(const CString &input, const CString &dual, int64 r1, int64 r2, const string &digitJoin, const string &digitJoinRev, int digitJoinMod28, int digitJoinRevMod28, int major, int minor)
{
...
void Ct3Dlg::AddSelections(const CString &input, const CString &dual, const row &r)
{
...
bool Ct3Dlg::CheckInputIntegrity(const CString &input)
{
	for (int i = 0; i < input.GetLength(); i++)
	{
		for (int j = 0; j < _countof(table); j++)
